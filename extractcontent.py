...
class ExtractContent(object):
    ...
    default = {
        ...
        "punctuations"       : r"(?is)(\343\200[\201\202]|\357\274[\201\214\216\237]|\.[^A-Za-z0-9]|,[^0-9]|!|\?)", # punctuation characters
        "waste_expressions"  : r"(?i)Copyright|All Rights Reserved",                 # characteristic keywords including footer
    ...
    def analyse(self, html, opt=None):
        ...
        if re.search(r"(?i)<\/frameset>|<meta\s+http-equiv\s*=\s*[\"']?refresh['\"]?[^>]*url",html) != None:
        ...
        if re.search(r"(?is)<!--\s*google_ad_section_start[^>]*-->",html) != None:
        ...
        self.title = title
        html = re.sub(r"(?s)(<h\d\s*>\s*(.*?)\s*</h\d\s*>)", self._estimate_title, html)
        ...
        score = 0
        bodylist = []
        list = re.split(r"</?(?:div|center|td)[^>]*>|<p\s*[^>]*class\s*=\s*[\"']?(?:posted|plugin-\w+)['\"]?[^>]*>",html)
        for block in list:
            ...
            c = (len(notlinked) + self._count_pattern(notlinked, opt["punctuations"]) \
                    * opt["punctuation_weight"]) * factor
            factor *= opt["decay_factor"]
            not_body_rate = self._count_pattern(block,opt["waste_expressions"]) \
                    + self._count_pattern(block,r"amazon[a-z0-9\.\/\-\?&]+-22") / 2.0
            if not_body_rate > 0:
                c *= (0.72 ** not_body_rate) 
            c1 = c * continuous
            if opt["debug"]:
                print "----- %f*%f=%f %d \n%s" % (c, continuous, c1, len(notlinked), self._strip_tags(block)[0:100])
        ...
        #return (self._strip_tags(body[0]), title)
        self.body = body[0]
    ...
    def _eliminate_useless_tags(self, html):
        ...
        html = re.sub(r"\342(?:\200[\230-\235]|\206[\220-\223]|\226[\240-\275]|\227[\206-\257]|\230[\205\206])","", html)
        html = html.decode('utf-8')
        # Eliminate useless html tags
        html = re.sub(r"(?is)<(script|style|select|noscript)[^>]*>.*?</\1\s*>","", html)
        html = re.sub(r"(?s)<!--.*?-->","", html)
        html = re.sub(r"<![A-Za-z].*?>/s","", html)
        html = re.sub(r"(?s)<div\s[^>]*class\s*=\s*['\"]?alpslab-slide[\"']?[^>]*>.*?</div\s*>","", html)
        html = re.sub(r"(?is)<div\s[^>]*(id|class)\s*=\s*['\"]?\S*more\S*[\"']?[^>]*>","", html)
        return html
